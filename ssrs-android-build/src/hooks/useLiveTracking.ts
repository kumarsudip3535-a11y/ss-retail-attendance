...
export function useLiveTracking(
  ...
  /**
   * The shared "got a GPS reading, now record or queue it" path — used by
   * both the web polling loop (collectPoint below) and the native
   * Android foreground-service watcher (handleNativePoint below). Pulled
   * out so the two very different *sources* of a reading (a one-shot
   * getCurrentPosition callback vs. an ongoing native watcher push) still
   * go through exactly one, already-tested Firestore/offline-queue path —
   * see nativeBackgroundTracking.ts's doc comment for why a second source
   * exists at all.
   */
  const recordOrQueuePoint = useCallback(
    async (raw: {
      lat: number
      lng: number
      ...
      const quality: LocationPointQuality =
        raw.accuracy != null && raw.accuracy > LOW_ACCURACY_THRESHOLD_METERS ? 'low_accuracy' : 'good'
  ...
  /** Web path: one on-demand reading via the browser Geolocation API, triggered by scheduleNext's timer. */
  ...
  /**
   * Native path: pushed continuously by the Android foreground-service
   * watcher — including with the screen off / app backgrounded, which is
   * the entire reason this second path exists. See
   * nativeBackgroundTracking.ts.
   */
  const handleNativePoint = useCallback(
    (point: NativePoint) => {
      void recordOrQueuePoint({
        lat: point.lat,
        lng: point.lng,
        ...
        timestamp: point.timeMs != null ? Timestamp.fromMillis(point.timeMs) : Timestamp.now(),
      })
    },
    [recordOrQueuePoint]
  )
  ...
  const start = useCallback(
    async (startEmployeeId: string, attendanceLogId: string) => {
      ...
      if (isNativeAndroid()) {
        // Native foreground-service watcher — one push-based subscription
        // for the whole session, instead of the web path's repeating
        // timer. This is the actual fix: collection keeps running with
        // the screen off / app backgrounded, which a JS timer cannot.
        const batteryPercent = batteryPercentRef.current
        const minIntervalMs =
          batteryPercent !== null && batteryPercent < LOW_BATTERY_THRESHOLD_PERCENT
            ? LOW_BATTERY_INTERVAL_MS
            : BASE_INTERVAL_MS
        await startNativeBackgroundTracking(minIntervalMs, handleNativePoint, (error) => {
          console.error('[useLiveTracking] Native background watcher error:', error)
        })
        return
      }
    ...
    [collectPoint, scheduleNext, handleNativePoint]
  )
  ...
  const resumeIfActive = useCallback(
    ...
    [isTracking, scheduleNext, flushOfflineQueue, handleNativePoint]
  )
  ...
  const stop = useCallback(async () => {
    ...
    // No-op when the native watcher was never started (web path, or
    // native but tracking wasn't active) — see its internal guard.
    await stopNativeBackgroundTracking().catch((error) => {
      console.error('[useLiveTracking] Failed to stop native background watcher:', error)
    })
